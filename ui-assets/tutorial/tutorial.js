(function() {
  ...
  angular.module('qwQuery').controller('tutorialController', tutorialController);

  tutorialController.$inject = ['qwQueryService','$http','qwQueryService','$timeout'];

  function tutorialController (qwQueryService,$http,qwQueryService,$timeout) {

    var tc = this;
    var max = guessmax();
    tc.current_slide = 1;
    tc.current_slide_content = "";
    tc.current_query = "";
    tc.next = next;
    tc.prev = prev;

    function init() {
      load(tc.current_slide);
      $timeout(function() {qwQueryService.addNewQueryAtEndOfHistory(tc.current_query);qwQueryService.getResult().query = tc.current_query;},
          250);
    }

    function slideUrl(n) {
      // note - .md is preprocessed into .html
      return 'tutorial/slide-' + n + '.md';
    }
    ...
    function load(n) {
      ...
      .then(function success(resp) {
        ...
        tc.current_slide_content = lesson.substring(0,lesson.indexOf('<pre id="example">'));
        tc.current_query = lesson.substring(lesson.indexOf('<pre id="example">')+18,lesson.indexOf('</pre>'));
        qwQueryService.getResult().query = tc.current_query;
        //console.log("Got query: " + tc.current_query);
      })
      .then(function error(resp) {
        //console.log("Got error response: " + resp);
      });
    }

    function next() {
      if (tc.current_slide < max) load(tc.current_slide + 1);
    }

    function prev() {
      if (tc.current_slide > 1) load(tc.current_slide - 1);
    }
