...
impl Command for SubDocGet {
    ...
    fn signature(&self) -> Signature {
        Signature::build("sub-doc get")
            .required(
                "path",
                SyntaxShape::String,
                "the path to be fetched from the documents",
            )
    ...
    fn usage(&self) -> &str {
        "Fetches the value of the provided path in a document through the data service"
    }
    ...
    fn run(
        ...
        run_sub_doc_get(self.state.clone(), engine_state, stack, call, input)
    }

    //TO DO - add examples once interface has been decided upon
}
...
fn run_sub_doc_get(
    ...
    for identifier in cluster_identifiers {
        ...
        debug!("Running kv sub-doc get for docs {:?} on field ", &ids);
        ...
        for ids in all_ids.clone() {
            for id in ids {
                ...
                workers.push(async move {
                    client
                        .request(
                            KeyValueRequest::SubDocGet { key: id, path: p },
                            cid,
                            deadline,
                            ctrl_c,
                        )
